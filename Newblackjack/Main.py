...
def BlackJackGame():
    ...
    # Set window dimensions (e.g., 80% of screen size)
    ...
    # True Count and AI variables
    running_count = 0
    true_count = 0

    def card_count_value(card):
        """Returns the card's value for Hi-Lo count system."""
        if card in [1, 10, 11, 12, 13]:  # Ace, 10, Face cards
            return -1
        elif card in [2, 3, 4, 5, 6]:  # Low cards
            return 1
        else:  # Neutral cards (7, 8, 9)
            return 0

    def update_true_count():
        """Calculates and updates the true count."""
        nonlocal true_count
        remaining_decks = deck.length() / 52
        true_count = round(running_count / remaining_decks, 2)
        true_count_label.config(text=f"True Count: {true_count}")

    def basic_strategy(player_score, dealer_card_val):
        """Provides basic strategy suggestions."""
        if player_score >= 17:
            return "Stand"
        elif 13 <= player_score <= 16:
            return "Stand" if dealer_card_val <= 6 else "Hit"
        elif player_score == 12:
            return "Stand" if 4 <= dealer_card_val <= 6 else "Hit"
        elif player_score == 11:
            return "Double Down"
        elif player_score == 10:
            return "Double Down" if dealer_card_val < 10 else "Hit"
        elif player_score == 9:
            return "Double Down" if 3 <= dealer_card_val <= 6 else "Hit"
        else:
            return "Hit"
    ...
    def getCardString(s,v):
        if(v == 1):
            ...
            valString = str(v)           
        fileName = f"cards/{valString}_of_{s.lower()}.png"
        addImage(fileName)

    def addImage(fileName):
        card = Image.open(fileName)
        cim = card.resize((95,145), Image.Resampling.LANCZOS)
